...
def evaluate(groundtruth, prediction, tiou_threshold=0.5):
    ...
    print('{:.3f}\t {:.3f}\t {:.3f}'.format(acc_sub*100, acc_obj*100, acc*100))
    # print("R@1: {:.3f}".format(acc*100))
    # i = 0
    # if(i == 0):
    #     print(groundtruth.items())
    # i = i+1


def main():

    groundtruth_dir = '/data1//VRG/VRC/VRG_eval/'
    gt_file = osp.join(groundtruth_dir, 'gt_out_vid_vrd.json')

    result_dir = '/data1/VRG/VRC/VRG_eval/'
    res_file = osp.join(result_dir, 'gt_out_stgcn.json')
